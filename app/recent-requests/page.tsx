...
const RequestCard = ({ request }: { request: BloodRequest }) => {
  ...
  const renderModal = () => {
    ...
    const modalContent = (
      ...
      >
        <div
          className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-8 w-full max-w-md border border-gray-700 shadow-xl animate-in fade-in-zoom-in"
          style={{ margin: "auto" }}
        ...
        >
          ...
          <div className="flex justify-center mb-6">
            {[1, 2, 3, 4, 5].map((star) => (
              <button
                ...
                className={`text-4xl transition-all duration-200 mx-1 ${
                  star <= rating
          ...
          <Textarea
            ...
            className="bg-gray-800/50 border-gray-700 focus:border-red-500 focus:ring-red-500 text-white placeholder-gray-400 rounded-lg mb-6"
            rows={3}
          />
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Button
              ...
              className="w-full border-gray-700 text-gray-300 hover:text-white hover:border-gray-600 h-12 text-base"
            >
              Cancel
            </Button>
          </div>
    ...
    // Use portal to render the modal outside the card component
    if (isBrowser) {
      return createPortal(
        modalContent,
  ...
  const renderSuccessMessage = () => {
    ...
    // Use portal to render the success message
    if (isBrowser) {
      return createPortal(
        successContent,
        document.getElementById("modal-root") || document.body
  ...
  const handleCompleteRequest = async () => {
    try {
      const response = await fetch(`/api/blood-requests/${request._id}`, {
        method: "PUT",
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-2">
        ...
        {/* Edit Button */}
        {user?.id === request.requestedBy.clerkID && request.isPending && (
          <Button
            size="sm"
            variant="ghost"
